...
const ReportsManager = () => {
  ...
  const handleSubmit = async () => {
    ...
    setForm({ title: "", report_type: "annual", file_url: "", year: new Date().getFullYear(), description: "" });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3"><FileText className="h-6 w-6 text-primary" /><h1 className="text-2xl font-bold font-heading">রিপোর্ট ম্যানেজার</h1></div>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEditing(null); }}>
          <DialogTrigger asChild><Button className="gap-2"><Plus className="h-4 w-4" /> রিপোর্ট যোগ</Button></DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <Select value={form.report_type} onValueChange={(v) => setForm({ ...form, report_type: v })}>
                ...
                <SelectContent>
                  <SelectItem value="annual">বার্ষিক রিপোর্ট</SelectItem>
                  <SelectItem value="financial">আর্থিক রিপোর্ট</SelectItem>
                  <SelectItem value="audit">অডিট রিপোর্ট</SelectItem>
                  <SelectItem value="project">প্রকল্প রিপোর্ট</SelectItem>
                </SelectContent>
              </Select>
              <Input type="number" placeholder="সাল" value={form.year} onChange={(e) => setForm({ ...form, year: Number(e.target.value) })} />
              <Input placeholder="ফাইল URL (PDF)" value={form.file_url} onChange={(e) => setForm({ ...form, file_url: e.target.value })} />
          ...
          <TableBody>
            {items.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell>{r.file_url ? <a href={r.file_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center gap-1"><ExternalLink className="h-3 w-3" /> দেখুন</a> : "-"}</TableCell>
