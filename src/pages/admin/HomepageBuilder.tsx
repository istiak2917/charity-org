...
interface HomepageSection { id: string; section_key: string; title: string; is_visible: boolean; display_order: number; config: any; }

const HomepageBuilder = () => {
  const { items, loading, update } = useAdminCrud<HomepageSection>({ table: "homepage_sections", orderBy: "display_order", ascending: true });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        {items.length === 0 && <div className="text-center text-muted-foreground py-12">কোনো সেকশন নেই। SQL স্ক্রিপ্ট রান করুন।</div>}
      </div>
    </div>
  );
};
