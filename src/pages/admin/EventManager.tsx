...
interface Event { id: string; title: string; description: string; location: string; event_date: string; image_url: string; is_featured: boolean; }
...
const EventManager = () => {
  ...
  const handleEdit = (e: Event) => {
    setEditing(e);
    setForm({ title: e.title, description: e.description || "", location: e.location || "", event_date: e.event_date || "", image_url: e.image_url || "", is_featured: e.is_featured });
    setOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <Table>
          <TableHeader><TableRow><TableHead>ইভেন্ট</TableHead><TableHead>স্থান</TableHead><TableHead>তারিখ</TableHead><TableHead className="text-right">অ্যাকশন</TableHead></TableRow></TableHeader>
          <TableBody>
            {items.map((e) => (
              <TableRow key={e.id}>
            ...
            {items.length === 0 && <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">কোনো ইভেন্ট নেই</TableCell></TableRow>}
          </TableBody>
        </Table>
      </Card>
    </div>
